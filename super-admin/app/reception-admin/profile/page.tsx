...
export default function ReceptionistProfilePage() {
  ...
  const { data: profile, isLoading, isError } = useMyReceptionistProfile();
  const updateMutation = useUpdateMyReceptionistProfile();
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const payload: Partial<ReceptionistProfileData> = {
    ...
    return (
      <div className="space-y-6">
        <PageHeader
          ...
          description={isCompletionMode ? "Please complete your information to access the reception dashboard." : "Update your profile information"}
          icon={UserCheck}
        >
        ...
        {isCompletionMode && (
          <div className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4 text-sm text-amber-700 dark:text-amber-400">
            You must complete your profile before accessing the reception dashboard.
          </div>
        )}
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1">{user?.email || "—"}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-400">Phone</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1">{user?.name || "—"}</p>
                </div>
                <div className="sm:col-span-2">
                  <FormField label="Emergency Contact" required error={formErrors.emergencyContact}>
                    <input
                      value={form.emergencyContact}
                      onChange={(e) => updateField("emergencyContact", e.target.value)}
                      placeholder="e.g. Spouse: +8801XXXXXXXXX"
                      className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600"
                    />
                  </FormField>
                </div>
              </div>
  ...
  if (isError || !profile) {
    return (
      <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 p-12 text-center text-gray-400">
        Receptionist profile not found. Please complete your profile.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6 flex flex-col items-center text-center">
          <div className="mb-4 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-emerald-600 to-[#76BC21] text-white text-3xl font-bold shadow-lg">
          ...
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{user?.name || "Receptionist"}</h2>
            {profile.receptionistCode && (
              <Badge variant="success">{profile.receptionistCode}</Badge>
            )}
          </div>
          <p className="text-sm text-emerald-600 dark:text-emerald-400 font-medium mt-0.5">{profile.department}</p>
          <Badge variant="success" className="mt-3">Active</Badge>

          <div className="mt-6 w-full space-y-3 border-t border-gray-100 dark:border-gray-800 pt-5">
            <ProfileInfoItem icon={<Mail className="h-4 w-4 text-emerald-600" />} label="Email" value={user?.email || "—"} />
            <ProfileInfoItem icon={<Phone className="h-4 w-4 text-emerald-600" />} label="Phone" value={user?.name || "—"} />
            <ProfileInfoItem icon={<PhoneCall className="h-4 w-4 text-emerald-500" />} label="Emergency Contact" value={profile.emergencyContact || "—"} />
          </div>
        </div>
