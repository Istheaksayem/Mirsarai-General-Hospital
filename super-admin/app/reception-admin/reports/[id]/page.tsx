...
import { ArrowLeft, Printer, Download, User, FlaskConical, Calendar, Stethoscope, FileText, Building2 } from "lucide-react";
import { useReports } from "@/lib/hooks/useReports";
import { useReportDownload } from "@/lib/hooks/useReportDownload";
...
export default function ViewReceptionReportPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const { handlePrint, handleDownload, isPrinting, isDownloading } = useReportDownload(report ?? null, printRef);
    ...
    <div className="space-y-6 w-full">
      <div className="flex items-center justify-between no-print">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push("/reception-admin/reports")} className="flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" onClick={handlePrint} loading={isPrinting} disabled={isDownloading}>
            <Printer className="h-3.5 w-3.5" /> Print
          </Button>
          <Button size="sm" variant="outline" onClick={handleDownload} loading={isDownloading} disabled={isPrinting}>
