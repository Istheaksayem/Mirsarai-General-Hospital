...
const columns: Column<Record<string, unknown>>[] = [
  { key: "id", header: "Report ID", cell: (r) => <span className="font-mono text-xs text-gray-500">{r.id as string}</span> },
  {
  ...
  { key: "completedDate", header: "Completed", cell: (r) => <span className="text-gray-500 dark:text-gray-400">{(r.completedDate as string) || "—"}</span> },
  {
    key: "status", header: "Status",
    cell: (r) => {
      const s = r.status as keyof typeof statusVariant;
...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Lab Reports" description={`${data.length} reports`} icon={BarChart3} />
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Pending", value: data.filter(r => r.status === "pending").length, color: "text-amber-500" },
          { label: "In Progress", value: data.filter(r => r.status === "in-progress").length, color: "text-blue-500" },
          { label: "Completed", value: data.filter(r => r.status === "completed").length, color: "text-green-600 dark:text-green-400" },
        ].map((s) => (
