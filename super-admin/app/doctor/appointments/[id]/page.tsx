...
import { ArrowLeft, User, Stethoscope, Calendar, Clock, FileText, Activity, Phone } from "lucide-react";
import { useDoctorAppointmentById, useUpdateDoctorAppointmentStatus } from "@/lib/hooks/useCmsAppointments";
...
export default function DoctorViewAppointmentPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();
  const { data, isLoading } = useDoctorAppointmentById(id);
    ...
    <div className="space-y-6 w-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push("/doctor/appointments")} className="flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
