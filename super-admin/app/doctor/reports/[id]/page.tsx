...
import { ArrowLeft, User, FlaskConical, Calendar, Stethoscope, FileText, Building2 } from "lucide-react";
import { useReportById } from "@/lib/hooks/useReports";
...
export default function DoctorViewReportPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    <div className="space-y-6 w-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push("/doctor/reports")} className="flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
