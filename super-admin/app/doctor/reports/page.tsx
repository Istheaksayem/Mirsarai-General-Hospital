...
const columns: Column<Record<string, unknown>>[] = [
  ...
  { key: "status", header: "Status", cell: (r) => { const s = r.status as keyof typeof sv; return <Badge variant={sv[s] ?? "default"}>{r.status as string}</Badge>; } },
];

export default function DoctorReportsPage() {
  const { data = [], isLoading } = useReports();
  const [search, setSearch] = useState("");
  const filtered = data.filter((r) => !search || r.patientName.toLowerCase().includes(search.toLowerCase()) || r.testName.toLowerCase().includes(search.toLowerCase())) as unknown as Record<string, unknown>[];
  return (
