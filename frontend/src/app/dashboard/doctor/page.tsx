...
import {
  FiGrid, FiCalendar, FiList, FiUser, FiClock,
  FiFileText, FiActivity, FiStar, FiSun, FiMoon,
...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    title: "Appointments",
    modules: [
      { key: "Today's Appointments", icon: <FiCalendar size={16} /> },
      { key: "All Appointments",     icon: <FiList size={16} /> },
    ],
  },
  {
    title: "Records",
    modules: [
      { key: "Patient History",    icon: <FiClock size={16} /> },
      { key: "Prescription Upload", icon: <FiFileText size={16} /> },
      { key: "Medical Reports",    icon: <FiActivity size={16} /> },
    ],
  },
  {
    title: "Planning",
    modules: [
      { key: "Schedule", icon: <FiStar size={16} /> },
    ],
  },
];
...
export default function DoctorDashboardPage() {
  ...
  const doctorName = (session as any)?.user?.name || "Doctor";
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-[#060913] flex text-slate-800 dark:text-slate-200 transition-colors duration-300 font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 bg-white dark:bg-[#070b13] border-r border-slate-200/50 dark:border-slate-800/40 flex flex-col justify-between transition-all duration-300 xl:translate-x-0 xl:static ${mobileSidebarOpen ? "translate-x-0" : "-translate-x-full"} ${sidebarCollapsed ? "w-20" : "w-64"}`}>
            ...
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              {!sidebarCollapsed && (
                <div className="leading-tight">
                  ...
                  <span className="text-[9px] text-[#1E2B7A] dark:text-[#76BC21] font-black tracking-widest block">DOCTOR PORTAL</span>
                </div>
              )}
            </div>
        ...
        <div className="p-3.5 border-t border-slate-200/50 dark:border-slate-800/40 bg-slate-50/30 dark:bg-slate-900/5 flex items-center justify-between">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-[#1E2B7A] to-[#76BC21] text-white flex items-center justify-center font-extrabold text-sm shadow-sm shrink-0">{doctorInitial}</div>
            {!sidebarCollapsed && (
              <div className="leading-tight truncate">
                <h4 className="text-xs font-bold text-slate-800 dark:text-slate-100 truncate">{doctorName}</h4>
                <span className="text-[9px] text-slate-400 dark:text-slate-500 font-bold block uppercase tracking-wider mt-0.5">Doctor</span>
              </div>
            )}
          </div>
          {!sidebarCollapsed && (
      ...
      <div className="flex-grow flex flex-col min-w-0 h-screen overflow-y-auto bg-slate-50/50 dark:bg-[#060913]">
        ...
        <header className="sticky top-0 z-20 h-20 bg-white/70 dark:bg-[#060913]/70 backdrop-blur-md border-b border-slate-200/40 dark:border-slate-800/40 flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="pl-3 border-l border-slate-200 dark:border-slate-800 flex items-center gap-2">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-[#1E2B7A] to-[#76BC21] text-white font-extrabold text-xs flex items-center justify-center shadow-md">{doctorInitial}</div>
              <button onClick={() => signOut({ callbackUrl: "/login" })} className="p-1.5 text-slate-400 hover:text-rose-500 transition" title="Sign Out">
                <FiLogOut size={14} />
              </button>
            </div>
          </div>
        ...
        <main className="flex-grow p-6 md:p-8 space-y-6 pb-12">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 font-bold mt-1.5">
                Workspace: <span className="text-[#1E2B7A] dark:text-[#76BC21] font-extrabold">Doctor Portal</span>
              </p>
            </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <DoctorModules activeModule={activeModule} token={token} />
            </motion.div>
          </AnimatePresence>
        </main>
      </div>
